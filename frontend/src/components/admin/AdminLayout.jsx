...
const AdminLayout = ({ children }) => {
  ...
  return (
    <div className="admin-shell [display:flex] [min-height:100vh] [background:var(--color-background)]">
      {/* Sidebar */}
      <aside className={cn(
        'sidebar-fixed fixed top-0 z-50 flex h-screen w-[260px] shrink-0 flex-col border-r border-[var(--admin-sidebar-border)] bg-[var(--admin-sidebar-bg)] transition-transform duration-300',
        ...
        {/* Logo */}
         <div className="[padding:1.25rem_1.5rem] [border-bottom:1px_solid_var(--admin-sidebar-border)] [display:flex] [align-items:center] [justify-content:space-between]">
           <Link to="/" className="[text-decoration:none]">
             <div className="brand-logo admin-sidebar-brand [font-family:var(--font-heading)] [font-weight:400] [font-size:1.4rem] [color:#FFFFFF] [font-style:italic] [letter-spacing:1px]">NgiNep.</div>
           </Link>
           <span className="admin-sidebar-kicker [color:var(--admin-sidebar-muted)] [font-family:var(--font-body)] [font-weight:400] [font-size:0.65rem] [text-transform:uppercase] [letter-spacing:1.5px]">Admin</span>
         </div>

        {/* User Info */}
        <div className="admin-sidebar-user [padding:1.25rem_1.5rem] [border-bottom:1px_solid_var(--admin-sidebar-border)] [background:var(--admin-sidebar-user-bg)]">
          <div className="admin-sidebar-user-name [font-family:var(--font-heading)] [font-weight:400] [font-size:1.1rem] [color:var(--admin-sidebar-text)]">{user?.first_name} {user?.last_name}</div>
          <div className="admin-sidebar-user-email [color:var(--admin-sidebar-muted)] [font-weight:300] [font-size:0.75rem] [margin-bottom:0.5rem]">{user?.email}</div>
          <span className="badge badge-yellow [font-size:0.65rem]">{user?.role?.replace('ROLE_', '')}</span>
        </div>
      ...
      {/* Main Content */}
      <div className="admin-content ml-[260px] flex flex-1 flex-col max-md:ml-0">
        {/* Top Bar */}
        <div className="admin-topbar [background:var(--color-surface)] [border-bottom:1px_solid_var(--color-accent)] [padding:1rem_2rem] [display:flex] [align-items:center] [gap:1rem] [position:sticky] [top:0] [z-index:30]">
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="show-mobile-admin hidden cursor-pointer rounded-lg border border-[var(--color-accent)] bg-transparent p-2 max-md:block">
            <Menu size={16} />
          </button>
        ...
        {/* Page Content */}
        <main className="admin-page-main [flex:1] [padding:2.5rem_2rem]">
          {children}
        </main>
      </div>
